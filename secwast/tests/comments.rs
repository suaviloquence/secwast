use secwast::parser::{self, Parse, ParseBuffer, Parser, Result};
...
(module)
    ...
    let d: Documented<secwast::core::Module> = parser::parse(&buf)?;
...
(func)
    ...
    let d: Documented<secwast::core::Func> = parser::parse(&buf)?;
    assert_eq!(
        d.comments.comments,
        vec![" this", " is\non\nmultiple", " lines"]
    );
